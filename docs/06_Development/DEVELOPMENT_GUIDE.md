...
所有已登录平台页面的主内容区统一显示浅色 `MSKPP&AIBench + 工号` 水印。登录页和对外展示页不加水印。新增功能、页面或显著修改展示方式时，必须在开发与评审阶段确认全局水印仍然有效，并检查是否存在脱离主内容层的特殊展示入口。

1. 普通页面默认复用 `AppLayout` 的全局水印，不得在新页面中绕开或遮挡该覆盖层。
2. 水印工号必须来自当前登录用户或服务端认证会话，不得信任 URL 参数等用户可修改的输入。
3. 必须检查该成果的所有展示入口，包括普通页面、卡片、图表、弹窗、全屏、iframe、独立新窗口和生成的 Viewer 文件。
4. 脱离 `AppLayout` 的 React 弹窗、全屏或 iframe 容器复用 `frontend/src/components/ResultWatermark.tsx`；独立 HTML 参照 Trace Viewer，由服务端认证响应注入水印。
5. 水印不得阻断点击、缩放、搜索等操作，也不能明显降低成果内容的可读性。
6. 新增特殊展示入口时，应补充水印内容和渲染路径测试。
7. PR 描述必须写明水印评审结论；决定不加水印时，也必须说明该位置不属于成果展示的理由。
