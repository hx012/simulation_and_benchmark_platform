...
新增功能、页面或显著修改成果展示方式时，必须在开发与评审阶段明确判断该位置是否需要水印，不能只检查现有页面。

1. 仿真结果、Benchmark 数据、性能分析、Trace、报告、团队成果，以及其他可能被截图复用的内部成果，必须显示 `MSKPP&AIBench + 工号` 水印。
2. 水印工号必须来自当前登录用户或服务端认证会话，不得信任 URL 参数等用户可修改的输入。
3. 必须检查该成果的所有展示入口，包括普通页面、卡片、图表、弹窗、全屏、iframe、独立新窗口和生成的 Viewer 文件。
4. React 成果区域复用 `frontend/src/components/ResultWatermark.tsx`；不继承 React 布局的独立 HTML，参照 Trace Viewer，由服务端认证响应注入水印。
